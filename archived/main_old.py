...
import util_old as utils

# Possible TODO: make setup a single or multiple functions in a separate file
...
if __name__ == '__main__':
    ...
    print(f"\nInput folder names: {input_folder_names}")
    print(f"\nSummary sheets: {summary_sheets}")

    print("\nBeginning matching process...\n")
                ...
                if special_case is not None:
                    ...
                    while input_data.cell(row = subitem_row, column = subitem_col + col_modifier).fill.start_color.index == 'FFDDEBF7':
                        ...
                        if isinstance(cell.value, str):
                            ...
                            print(f"write_data string trigger: {write_data}")
                            ...
                            if input_data.cell(row = subitem_row, column = subitem_col + col_modifier + 1).fill.start_color.index == 'FFDDEBF7':
                                write_data =+ ", "
                                print(f"write_data string comma trigger: {write_data}")
                        elif isinstance(cell.value, (int, float)):
                            write_data =+ cell.value
                            print(f"write_data number trigger: {write_data}")
                        else:
